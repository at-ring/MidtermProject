...
{
    class ComboLevel : Level
    {
        public ComboLevel(LevelManager thisLevelManager)
        ...
        public override void LoadLevel()
        {
            ...
            bool moveUp = true; // should the peg move up or down first?
            ...
            for (int row = 0; row < ThisLevelManager.NumberOfCompleteRows * 2 + 1; row++)
            {
                for (int col = 0; (row % 2 == 0 && col < 13) || (row % 2 == 1 && col < 12); col++)
                {
                    ...
                    if (row % 4 == 1)
                    {
                        movementVector = new Vector2(1.5f, 0f);
                    }
                    else if (row % 4 == 3)
                    {
                        movementVector = new Vector2(0f, 1.5f);                        
                    }
                    //Console.WriteLine("ml: {0}, mu: {1}, row: {2}, col: {3}", moveLeft, moveUp, row, col);
                    ThisLevelManager.ThisActorManager.AddSprite(new PegSprite(ThisLevelManager.PegTexture, new Vector2(colDrawPos, rowDrawPos), ThisLevelManager.PegSize, new Point(1, 1), new Vector2(0, 0), new Vector2(1, 1), row, new Vector2(ThisLevelManager.Spacing.X, ThisLevelManager.Spacing.Y / 2), movementVector, moveLeft, moveUp, moveHorizontal, moveVertical, true));
