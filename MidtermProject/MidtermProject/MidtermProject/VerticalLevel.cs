...
{
    class VerticalLevel : Level
    {
        public VerticalLevel(LevelManager thisLevelManager)
        ...
        public override void LoadLevel()
        {
            ...
            bool moveLeft = false; // should peg move right or left first?
            bool moveUp = false; // should the peg move up or down first?
            bool moveHorizontal = false; // should the peg move horizontal?
            ...
            for (int row = 0; row < ThisLevelManager.NumberOfCompleteRows * 2 + 1; row++)
            {
                ...
                {
                    ThisLevelManager.ThisActorManager.AddSprite(new PegSprite(ThisLevelManager.PegTexture, new Vector2(colDrawPos, rowDrawPos), ThisLevelManager.PegSize, new Point(1, 1), new Vector2(0, 0), new Vector2(1, 1), row, new Vector2(0, ThisLevelManager.Spacing.Y / 2), new Vector2(0, 1.5f), moveLeft, moveUp, moveHorizontal, moveVertical, false));
